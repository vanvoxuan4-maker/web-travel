...
export const EditTourModal: React.FC<EditTourModalProps> = ({
  ...
  // STEP 4: Refund Policy Presets & Custom List
  const [selectedPolicyPreset, setSelectedPolicyPreset] = useState<string>(() => {
    if (tour?.refundPolicy && tour.refundPolicy.length > 0) {
      const match = REFUND_POLICY_PRESETS.find(p => 
        p.rules.length === tour.refundPolicy.length &&
        p.rules[0]?.condition === tour.refundPolicy[0]?.condition
      );
  ...
  const [refundPolicyList, setRefundPolicyList] = useState<RefundPolicyItem[]>(() => {
    if (tour?.refundPolicy && tour.refundPolicy.length > 0) {
      return tour.refundPolicy.map(r => ({ ...r }));
    }
    return DEFAULT_REFUND_POLICY.map(r => ({ ...r }));
  });
  ...
  // STEP 4: FAQ Presets & Custom List
  const [selectedFaqPreset, setSelectedFaqPreset] = useState<string>(() => {
    if (tour?.faqs && tour.faqs.length > 0) {
      const match = FAQ_PRESETS.find(p => 
        p.faqs.length === tour.faqs.length &&
        p.faqs[0]?.q === tour.faqs[0]?.q
      );
      return match ? match.id : 'custom';
    }
    return 'general';
  ...
  const [faqList, setFaqList] = useState<FAQItem[]>(() => {
    if (tour?.faqs && tour.faqs.length > 0) {
      return tour.faqs.map(f => ({ ...f }));
    }
    return DEFAULT_FAQS.map(f => ({ ...f }));
  });
