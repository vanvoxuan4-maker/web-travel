...
const DEFAULT_FALLBACK_TOUR: Tour = {
  id: 'default-tour',
  code: 'WT-STANDARD-4N3D',
  sku: 'WT1001',
  title: 'Hành Trình Du Lịch Mẫu Tiêu Chuẩn',
  shortTitle: 'Tour Tiêu Chuẩn',
  ...
  leiScore: '90/100',
  esgScore: '88/100',
  hotelSpecs: { hotelName: 'Khách Sạn 4★', roomType: 'Phòng Deluxe', inclusions: [] },
  gallery: [],
  inclusionsList: [],
  exclusionsList: [],
  refundPolicy: [],
...
export const BudgetCalculator: React.FC<BudgetCalculatorProps> = ({ onOpenBooking }) => {
  const [tourId, setTourId] = useState(TOURS_DATA[0]?.id || DEFAULT_FALLBACK_TOUR.id);
  ...
  const tour = useMemo(() => TOURS_DATA.find(t => t.id === tourId) || TOURS_DATA[0] || DEFAULT_FALLBACK_TOUR, [tourId]);
