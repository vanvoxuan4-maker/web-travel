 ...
 */
export function getRemainingSeats(tourId, date) {
 ...
 */
export function getDatePrice(tourId, date) {
 ...
 */
export function getDateLabel(tourId, date) {
  const tour = TOURS_DATA.find(t => t.id === tourId);
 ...
 */
export function getDateDetails(tourId, date) {
  const tour = TOURS_DATA.find(t => t.id === tourId);
  if (!tour) return null;
 ...
 */
export function deductSeats(tourId, date, count) {
  const store = getInventoryStore();
